...
/**
 * An activity representing a single Measurement detail screen. This
 * activity is only used on narrow width devices. On tablet-size devices,
 * item details are presented side-by-side with a list of items
 * in a {@link MeasurementListActivity}.
 */
public class MeasurementDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            arguments.putString(MeasurementDetailFragment.ARG_ITEM_ID,
                    getIntent().getStringExtra(MeasurementDetailFragment.ARG_ITEM_ID));
            MeasurementDetailFragment fragment = new MeasurementDetailFragment();
            fragment.setArguments(arguments);
            getSupportFragmentManager().beginTransaction()
                    .add(R.id.measurement_detail_container, fragment)
                    .commit();
        }
    }
